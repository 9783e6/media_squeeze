...
class _FileChooserWidgetState extends State<FileChooserWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    if (result != null) {
      setState(() {
        appState.openCompressionMenu(context, File(result.files.single.path!));
      });
    }
  }
          ...
          onPerformDrop: (event) async {
            ...
            for (var item in event.session.items) {
              ...
              if (await reader!.canProvide(Formats.fileUri)) {
                reader!.getValue<Uri>(Formats.fileUri, (Uri? fileUri) {
                  if (fileUri != null) {
          ...
          child: Stack(
            children: [
              ...
              if (isDragging)
                Align(
                  alignment: Alignment.center,
                  child: Icon(Icons.file_upload, size: 50, color: Colors.blue[200]),
                ),
            ],
          ),
        ),
